...
def load_pickle(filepath):
    """Load pickled data"""
    documents_f = open('files/'+filepath+'.pickle', 'rb')
...
with train_graph.as_default():
    ...
    input_data, targets, lr, keep_prob, target_length, max_target_length, input_length = Seq2Seq.model_inputs()

    # Create the training and inference logits
    training_logits, inference_logits = Seq2Seq.seq2seq_model(tf.reverse(input_data, [-1]),
                                                            targets,
                                                            keep_prob,
                                                            input_length,
                                                            target_length,
...
with tf.Session(graph=train_graph) as sess:
    ...
    for epoch_i in range(1, epochs+1):
        ...
        for batch_i, (targets_batch, inputs_batch, targets_lengths, inputs_lengths) in enumerate(
                Seq2Seq.get_batches(dec_targets, enc_inputs, vocab2int, batch_size)):
            ...
            if batch_i > 0 and batch_i % update_check  == 0:
                ...
                if update_loss <= min(question_update_loss):
                    ...
                    if stop_early == stop:
                        break
                # losses.append(round(update_loss/update_check, 3))
                # arrepochs.append(len(losses))
                # plt.plot( arrepochs ,losses, 'ro')
                # plt.show()
                update_loss = 0
